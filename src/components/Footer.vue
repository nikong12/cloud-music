...
import { getMusicUrl, getLikeMusic, getMusicLyric } from '@/api/music'
...
export default {
    name: 'Footer',
    components: { PlayFooter, PlayDetail, Footer },
    computed: {
        ...mapState([
            'playId',
            'playList',
    ...
    methods: {
        ...
        changeCurrenTime(val) {
            ...
            this.$store.commit('setCurrenTime', time * 1000)
        },
        openLeave() {
            this.levelbox === false
                ? (this.levelbox = true)
        ...
        getDrawertow() {
            if (this.drawertow === false) {
                this.$store.commit('setDrawertow', true)
            } else {
